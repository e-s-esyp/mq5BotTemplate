...
public class NeuroMappingPartitionByDetermination extends NeuroMapping {

    public static int maxLevel = 20;
    public static int minImagesDefault = 25;
    public static double proportion = 0.30;
    ...
    public static boolean hasTesting = false;
    public static boolean restrictByLevel = false;
    ...
    private static class IndexedDetermination {
        ...
        public ImageDivision test = null;
    ...
    public static NeuroMap train(final NetImage[] images,
        ...
        if (hasTesting) {
            ...
            selectTrainTestOur(mapImages, trainImages, testImages, rnd);
            map.computationType.setWeights(map.getAverageOut(map.getKlassesSize(trainImages), trainImages.length));
            train(map, trainImages, testImages, rnd);
            if (map.rrError >= 1) {
                map.noTest = true;
                map.computationType.setWeights(map.getAverageOut(images));
                train(map, mapImages, mapImages, rnd);
            }
        } else {
            map.noTest = true;
            map.computationType.setWeights(map.getAverageOut(images));
    ...
    public static NeuroMap train(
        ...
        if (hasTesting) {
            final int numTrainImages = (int) (mapImages.length * proportion);
            final int numTestImages = mapImages.length - numTrainImages;
            final NeuroImage[] trainImages = new NeuroImage[numTrainImages];
            final NeuroImage[] testImages = new NeuroImage[numTestImages];
            selectTrainTestOur(mapImages, trainImages, testImages, rnd);
            map.computationType.setWeights(map.getAverageOut(map.getKlassesSize(trainImages), trainImages.length));
    ...
    private static void train(final NeuroMapPartition map,
        ...
        if (randomized) {
            trainStructureR(map, structure, maxLevel, minImages, trainImages, testImages, rnd);
        } else {
            trainStructure(map, structure, maxLevel, minImages, trainImages, testImages);
        }
...
//        return minImages;
    ...
    private static void trainStructure(
        ...
        structure.numKlasses = trainImages[0].numKlasses;
        final long[] a = map.getKlassesSize(trainImages);
        structure.average = map.getAverageOut(a, trainImages.length);
        map.setBestKlass(structure, trainImages, testImages);
        if (recursionIndex > 0 && trainImages.length >= minImages) {
            final IndexedDetermination determination = getBestDetermination(trainImages, testImages, a);
...
//-----------------------------------
            ...
            trainStructure(
                    ...
                    determination.test.notLess);
//---------------------------------
            structure.divided = map.isBetterThenAverage(structure, trainImages, testImages, recursionIndex);
        }
    }
    ...
    private static NeuroBranch trainStructureR(
        ...
        map.setBestKlass(averageStructure, trainImages, testImages);
        final IndexedDetermination bestDetermination = getBestDetermination(trainImages, testImages, a);
        if (recursionIndex > 0 && trainImages.length >= minImages) {
            final NeuroBranch[] structures = new NeuroBranch[]{
                    ...
                    trainStructureByDetermination(
                            map,
                            getRandomDetermination(trainImages, testImages, rnd),
                            averageStructure,
                            recursionIndex,
                            minImages,
                            trainImages,
            ...
            for (final NeuroBranch structure : structures) {
                structure.setError(map.computationType.errorTypeNum, trainImages, testImages, recursionIndex);
            }
...
//                        detl * detr * Math.pow(Math.min(detl, detr), 10);
        ...
        while (index < determinations.length) {
            ...
            result.test = divideImages(testImages, result.inIndex, result.divisor);
            result.setIfDivided();
    ...
    private static IndexedDetermination getBestDetermination4(
            final NeuroImage[] trainImages,
            final NeuroImage[] testImages,
            final long[] a) {
...
//            double maxProfit = Double.MIN_VALUE;
    ...
    private static IndexedDetermination getRandomDetermination(
        ...
        while (index < order.length && result.divided) {
            ...
            result.test = divideImages(testImages, inIndex, divisor);
            result.setIfDivided();
        }
        if (result.divided) {
            return result;
    ...
    private static void sortImages(
        ...
        Arrays.sort(images, new Comparator<NeuroImage>() {
            ...
            public int compare(final NeuroImage o1, final NeuroImage o2) {
                if (o1.in[bestInIndex] < o2.in[bestInIndex]) return -1;
                if (o1.in[bestInIndex] > o2.in[bestInIndex]) return 1;
                return 0;
            }
        });
